       ...
       if (!diceIsRotating && timer != 4.0f) // 如果色子停止旋转且不处于首回合开始前(首回合开始前色子会有个下落过程)
        {
           ...
           {
                if (nextPlaneNum <= DiceFaceUpNum)
                {
                    planeNum = planeNum + nextPlaneNum;
                    if (planeNum > 11) planeNum = 0;
                    newPosition = Planes.transform.GetChild(planeNum).position; // 取得落点坐标
                    transform.localPosition = Vector3.MoveTowards(transform.localPosition, newPosition, 8.0f); // 每秒10米进行移动
                    nextPlaneNum++;
                }
                else
                {
